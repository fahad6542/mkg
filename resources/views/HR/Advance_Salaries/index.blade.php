		...
		<div class="col-12">
			<div class="card">
				...
				<div class="card-header border-bottom">
					<h4 class="card-title">Add New Advance Salary</h4>
                    <a type="button" class="btn btn-relief-primary click_if_invalid"  href="{{route('Advance_salary.create')}}" id="">Add New</a>
				</div>
				<!-- Modal -->
...
$(document).ready(function() {
    ...
    table = $('.data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('Advance_salary.index') }}",
        columns: [
            {data: 'DT_RowIndex', name: 'DT_RowIndex'},
            {data: 'name', name: 'name'},
            {data: 'prevous_advance_salary', name: 'prevous_advance_salary'},
            {data: 'new_advance', name: 'new_advance'},
   ...
   $('#save_btn').click(function(e) {
    ...
    $('body').on('click', '.delete-btn', function () {
        ...
        if (result.value) {
            ...
            $.ajax({
                type: "DELETE",
                url: "{{ route('Advance_salary.store') }}"+'/'+id, //PK
